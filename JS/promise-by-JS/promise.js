function MyPromise(callback){
    ...
    this.then = function(onFulfilled,onReject){
        ...
        return new self.constructor(function(resolve,reject){
            ...
            //若执行完毕 就在then方法里面立即执行 否则就将参数记录下来，等待state就绪在执行函数
            if(state === true){
    ...
    function resolve(data){
        ...
        param = data;
        if(nextResolve){
            doAsynconFulfilled(asynconFulfilled,nextResolve,nextRejext)
        }
    }
    // reject方法
    function reject(err){
        ...
        param = err;
        if(nextRejext){
            doAsynconRejected(asynconReject,nextResolve,nextRejext)
        }
    }
    function doAsynconFulfilled(onFulfilled,resolve,reject){
        window.setTimeout(function(){
            ...
            if(typeof onFulfilled === 'function'){
                //执行onFulfilled方法获取返回值promise(param)
